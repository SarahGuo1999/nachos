...
#define FileNameMaxLen 		15	// for simplicity, we assume 
					// file names are <= 15(9 originally) characters long
...
class DirectoryEntry {
  public:
    ...
    char name[FileNameMaxLen + 1];	// Text name for file, with +1 for 
					// the trailing '\0'
    char PathName[FileNameMaxLen * 2];  // path of this file.
    bool IsDirectory;
};
...
class Directory {
  public:
    ...
    // int Directory::FindRootDirSector(char* file);
    int GetRootDirSector(char* name);
    void GetFileName(char* name, char* FileName);
    DirectoryEntry *GetTable(){ return this->table; }
