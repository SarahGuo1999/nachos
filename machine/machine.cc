...
{
    int i;
    pageTableSize = 128;  // tlb : 32
    #ifdef INVERTED
        pageTableSize = NumPhysPages;
    #endif
    bitmap = new BitMap(pageTableSize);
...
//----------------------------------------------------------------------
// Machine::MemoryAllocate/MemoryClear
//      Allocate and clear memory for current thread.
//----------------------------------------------------------------------

int Machine::MemoryAllocate(){
    int num = bitmap->Find();
    if(num != -1){
        pageTable[num].ThreadId = currentThread->getThreadId();
        /*printf("Allocate PageNumber %d in Memory    ", num);
        printf("Current Clear Page Number : %d\n", bitmap->NumClear());*/
    }
    return num;
}

void Machine::MemoryClear(){ 
    int CurPageNum;
    for(int i = 0;i < pageTableSize;++i){
        CurPageNum = pageTable[i].physicalPage;
        if(CurPageNum != -1 && bitmap->Test(CurPageNum) && \
            pageTable[i].ThreadId == currentThread->getThreadId()){
            /*printf("Clear    PageNumber %d in Memory    ", CurPageNum);
            printf("Current Clear Page Number : %d\n", bitmap->NumClear());*/
            bitmap->Clear(CurPageNum);
        }
    }
}

// void Machine::WritePage(){
//     OpenFile *VirtualMem = fileSystem->Open("VirtualMem");
//     if(VirtualMem == NULL)  
//         ASSERT(FALSE);
//     for(int i = 0; i < pageTableSize;++i){
//         if(pageTable[i].ThreadId == currentThread->getThreadId() && \
//             pageTable[i].dirty && pageTable[i].physicalPage != -1){
//                 VirtualMem->WriteAt(&(mainMemory[i*PageSize]),\
//                     PageSize, pageTable[i].virtualPage*PageSize);
//                 pageTable[i].valid = FALSE;
//                 /*printf("ThreadId:%d Write Dirty VirtualPage %d to Memory\n",\
//                         pageTable[i].ThreadId, pageTable[i].virtualPage);*/
