...
class Lock {
  ...
  private:
    ...
    // some plus stuff
    Semaphore* SemaphoreOfLock;   // Semaphore connect to this Lock
    Thread* OwnerThread;     // Thread which holds this lock
};
...
class Condition {
  ...
  private:
    char* name;
    // plus some stuff 
    List* queue;    // store waiting threads for this Condition
};
#endif // SYNCH_H
